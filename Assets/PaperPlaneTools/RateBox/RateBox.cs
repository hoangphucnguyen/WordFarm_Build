namespace PaperPlaneTools
{
	...
	public class RateBox 
	{
		...
		public void ForceShow(string title, string message, string rateButtonTitle, string postponeButtonTilte, string rejectButtonTitle = null)
		{
			Alert alert = new Alert (title, message)
				.SetPositiveButton (rateButtonTitle, () => {
                    ...
                    {
                        Flurry.Flurry.Instance.LogEvent("AppRate_Dialog_Rate");
                        Fabric.Answers.Answers.LogCustom("AppRate_Dialog_Rate");
                    }
			...
			if (rejectButtonTitle != null) 
			{
				alert.SetNegativeButton (rejectButtonTitle, () => {
                    if (!Debug.isDebugBuild)
                    {
                        Flurry.Flurry.Instance.LogEvent("AppRate_Dialog_Reject");
                        Fabric.Answers.Answers.LogCustom("AppRate_Dialog_Reject");
                    }
		...
		public bool CheckConditionsAreMet()
		{
			...
			if(Conditions.RequireInternetConnection && GetLocalIPAddress() == "0.0.0.0"){ 
				DebugLog(String.Format(
		...
		public void GoToRateUrl() {
            GameFramework.GameStructure.GameManager.SafeTriggerMessage(new AppRateMessage());
