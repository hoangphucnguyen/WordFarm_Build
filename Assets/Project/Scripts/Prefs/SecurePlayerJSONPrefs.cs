...
{
	/// <summary>
	/// A secured alternative to the <a href="http://docs.unity3d.com/Documentation/ScriptReference/JSONPrefs.html">PlayerPrefs</a> class.
	/// </summary>
	/// This class saves preferences values in an encrypted format.
	public static class SecurePlayerJSONPrefs
	{
		...
		public static float GetFloat(string key, float defaultValue = 0.0f, bool? useSecurePrefs = null)
		{
			// if not using secure prefs, or we provide an override then just fall through to standard call.
			if ((!UseSecurePrefs && !useSecurePrefs.HasValue) || (useSecurePrefs.HasValue && useSecurePrefs.Value == false))
				return JSONPrefs.GetFloat(key, defaultValue);
			...
			{
				var value = JSONPrefs.GetFloat(key, defaultValue);
				SetFloat(key, value);
				JSONPrefs.DeleteKey(key);
				return value;
			}
		...
		public static int GetInt(string key, int defaultValue = 0, bool? useSecurePrefs = null)
		{
			// if not using secure prefs, or we provide an override then just fall through to standard call.
			if ((!UseSecurePrefs && !useSecurePrefs.HasValue) || (useSecurePrefs.HasValue && useSecurePrefs.Value == false))
				return JSONPrefs.GetInt(key, defaultValue);
			...
			if (AutoConvertUnsecurePrefs && JSONPrefs.HasKey(key))
			{
				var value = JSONPrefs.GetInt(key, defaultValue);
				SetInt(key, value);
				JSONPrefs.DeleteKey(key);
				return value;
			}
