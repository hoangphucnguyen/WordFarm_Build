...
@interface ISN_ShareItemProvider : UIActivityItemProvider <UIActivityItemSource>
...
@implementation ISN_ShareItemProvider
...
- (id)item
{
    ...
    if ([self.activityType isEqualToString:UIActivityTypePostToFacebook] && text != nil) {
        ...
        for (NSTextCheckingResult *match in matches) {
            if ([match resultType] == NSTextCheckingTypeLink) {
                ...
                text = [text stringByReplacingOccurrencesOfString:[[match URL] absoluteString] withString:@""];
            }
        }
...
-(void) mediaShare:(NSString *)text  media:(NSString *)media {
    ...
    if(media.length != 0) {
        ...
        ISN_ShareItemProvider *provider = [[ISN_ShareItemProvider alloc] initWithText:text image:nil];
        ISN_ShareItemProvider *providerImage = [[ISN_ShareItemProvider alloc] initWithText:nil image:image];
        
        //[UIPopoverPresentationController alloc] ini
        [[ISN_NativeUtility sharedInstance] ISN_NativeLog: @"ISN: image added"];
        
        controller = [[UIActivityViewController alloc] initWithActivityItems:@[provider, providerImage] applicationActivities:nil];
        
    } else {
        [[ISN_NativeUtility sharedInstance] ISN_NativeLog: @"ISN: no media"];
        ISN_ShareItemProvider *provider = [[ISN_ShareItemProvider alloc] initWithText:text image:nil];
        
        controller = [[UIActivityViewController alloc] initWithActivityItems:@[provider] applicationActivities:nil];
    }
    ...
    [controller setCompletionWithItemsHandler:^(UIActivityType  _Nullable activityType, BOOL completed, NSArray * _Nullable returnedItems, NSError * _Nullable activityError) {
        if ( !completed ) {
            [[ISN_NativeUtility sharedInstance] ISN_NativeLog: @"ISN: media share, canceled "];
            UnitySendMessage("IOSSocialManager", "OnMediaSharePostFailed", [ISN_DataConvertor NSStringToChar:@""]);
            return;
        }
        
        if ( activityError ) {
            [[ISN_NativeUtility sharedInstance] ISN_NativeLog: @"ISN: media share, error: %@", activityError.localizedDescription];
            UnitySendMessage("IOSSocialManager", "OnMediaSharePostFailed", [ISN_DataConvertor NSStringToChar:@""]);
            return;
        }
        
        UnitySendMessage("IOSSocialManager", "OnMediaSharePostSuccess", [ISN_DataConvertor NSStringToChar:activityType == nil ? @"" : activityType]);
    }];
